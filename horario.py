...
while True:
    ...
    if option == "1":
        ...
        while True:
            ...
            # Validación estricta anti-letras y anti-números locos
            if len(start_time) != 5 or start_time[2] != ':' or not (start_time[:2].isdigit() and start_time[3:].isdigit()):
                print("Error: Formato inválido. No se permiten letras ni números largos. Use estrictamente HH:MM (ej. 14:00).")
                continue
            
            h_start, m_start = int(start_time[:2]), int(start_time[3:])
            if not (0 <= h_start <= 23 and 0 <= m_start <= 59):
                print("Error: La hora de inicio está fuera de rango (00:00 a 23:59).")
                continue
            ...
            if len(end_time) != 5 or end_time[2] != ':' or not (end_time[:2].isdigit() and end_time[3:].isdigit()):
                print("Error: Formato inválido. No se permiten letras ni números largos. Use estrictamente HH:MM (ej. 16:00).")
                continue

            h_end, m_end = int(end_time[:2]), int(end_time[3:])
            if not (0 <= h_end <= 23 and 0 <= m_end <= 59):
                print("Error: La hora de fin está fuera de rango (00:00 a 23:59).")
                continue
            ...
            if item["materia"] == subject_to_modify:
                ...
                while True:
                    ...
                    if len(new_start_time) != 5 or new_start_time[2] != ':' or not (new_start_time[:2].isdigit() and new_start_time[3:].isdigit()):
                        print("Error: Formato inválido. Use estrictamente HH:MM.")
                        continue
                    
                    ns_h, ns_m = int(new_start_time[:2]), int(new_start_time[3:])
                    if not (0 <= ns_h <= 23 and 0 <= ns_m <= 59):
                        print("Error: Hora de inicio fuera de rango.")
                        continue
                    ...
                    ne_h, ne_m = int(new_end_time[:2]), int(new_end_time[3:])
                    if not (0 <= ne_h <= 23 and 0 <= ne_m <= 59):
                        print("Error: Hora de fin fuera de rango.")
                        continue
        ...
        if not schedule:
            ...
            # Escritura asegurada al finalizar la paginación de la opción 5
